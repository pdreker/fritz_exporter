...
class FritzCapability(ABC):
    ...
    def checkCapability(self, device):
        ...
        logger.debug(f'Capability {type(self).__name__} set to '
                     '{self.present} on device {device.host}')
...
class FritzCapabilities():
    ...
    def merge(self, other_caps):
        for cap in self.capabilities:
            self.capabilities[cap].present = (self.capabilities[cap].present or
                                              other_caps.capabilities[cap].present)
...
class DeviceInfo(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['uptime'] = CounterMetricFamily(
            'fritz_uptime', 'FritzBox uptime, system info in labels',
...
class HostNumberOfEntries(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['numhosts'] = GaugeMetricFamily(
            'fritz_known_devices', 'Number of devices in hosts table',
...
class UserInterface(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['update'] = GaugeMetricFamily(
            'fritz_update_available', 'FritzBox update available',
    ...
    def _getMetricValues(self, device):
        ...
        new_software_version = update_result['NewX_AVM-DE_Version'] if (
            update_result['NewUpgradeAvailable']) else 'n/a'
        self.metrics['update'].add_metric(
            [device.serial, device.friendly_name, new_software_version],
            upd_available
        )
...
class LanInterfaceConfigStatistics(FritzCapability):
    ...
    def _getMetricValues(self, device):
        ...
        self.metrics['lanbytes'].add_metric(
            [device.serial, device.friendly_name, 'rx'], lanstats_result['NewBytesReceived'])
        self.metrics['lanbytes'].add_metric(
            [device.serial, device.friendly_name, 'tx'], lanstats_result['NewBytesSent'])
        self.metrics['lanpackets'].add_metric(
            [device.serial, device.friendly_name, 'rx'], lanstats_result['NewPacketsReceived'])
        self.metrics['lanpackets'].add_metric(
            [device.serial, device.friendly_name, 'tx'], lanstats_result['NewPacketsSent'])
...
class WanDSLInterfaceConfig(FritzCapability):
    ...
    def _getMetricValues(self, device):
        ...
        self.metrics['enable'].add_metric(
            [device.serial, device.friendly_name],
            fritz_dslinfo_result['NewEnable'])
...
class WanDSLInterfaceConfigAVM(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['fec'] = CounterMetricFamily(
            'fritz_dsl_fec_errors_count', 'Number of Forward Error Correction Errors',
            labels=['serial', 'friendly_name'])
        self.metrics['crc'] = CounterMetricFamily(
            'fritz_dsl_crc_errors_count', 'Number of CRC Errors',
            labels=['serial', 'friendly_name'])
    ...
    def _getMetricValues(self, device):
        ...
        self.metrics['fec'].add_metric(
            [device.serial, device.friendly_name],
            fritz_avm_dsl_result['NewFECErrors'])
        self.metrics['crc'].add_metric(
            [device.serial, device.friendly_name],
            fritz_avm_dsl_result['NewCRCErrors'])
...
class WanPPPConnectionStatus(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['uptime'] = CounterMetricFamily(
            'fritz_ppp_connection_uptime', 'PPP connection uptime',
            labels=['serial', 'friendly_name'], unit='seconds')
        self.metrics['connected'] = GaugeMetricFamily(
            'fritz_ppp_connection_state', 'PPP connection state',
    ...
    def _getMetricValues(self, device):
        ...
        self.metrics['uptime'].add_metric(
            [device.serial, device.friendly_name],
            fritz_pppstatus_result['NewUptime'])
        self.metrics['connected'].add_metric(
            [device.serial, device.friendly_name, fritz_pppstatus_result['NewLastConnectionError']],
            pppconnected)
        yield self.metrics['uptime']
...
class WanCommonInterfaceConfig(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['wanconfig'] = GaugeMetricFamily(
            'fritz_wan_max_bitrate', 'max bitrate at the physical layer',
            labels=['serial', 'friendly_name', 'wantype', 'direction'], unit='bps')
        self.metrics['wanlinkstatus'] = GaugeMetricFamily(
            'fritz_wan_phys_link_status', 'link status at the physical layer',
    ...
    def _getMetricValues(self, device):
        ...
        self.metrics['wanconfig'].add_metric(
            [device.serial, device.friendly_name, wanstatus_result['NewWANAccessType'], 'tx'],
            wanstatus_result['NewLayer1UpstreamMaxBitRate'])
        self.metrics['wanconfig'].add_metric(
            [device.serial, device.friendly_name, wanstatus_result['NewWANAccessType'], 'rx'],
...
class WanCommonInterfaceDataBytes(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['wanbytes'] = CounterMetricFamily(
            'fritz_wan_data', 'WAN data in bytes',
            labels=['serial', 'friendly_name', 'direction'], unit='bytes')
...
class WanCommonInterfaceByteRate(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['wanbyterate'] = GaugeMetricFamily(
            'fritz_wan_datarate', 'Current WAN data rate in bytes/s',
            labels=['serial', 'friendly_name', 'direction'], unit='bytes')
...
class WanCommonInterfaceDataPackets(FritzCapability):
    ...
    def createMetrics(self):
        self.metrics['wanpackets'] = CounterMetricFamily(
            'fritz_wan_data_packets', 'WAN data in packets',
            labels=['serial', 'friendly_name', 'direction'], unit='count')
...
class WlanConfigurationInfo(FritzCapability):
    ...
    def checkCapability(self, device):
        for wlan in range(len(self.wifi_present)):
            ...
            logger.debug(
                f'WLANCapability {type(self).__name__} '
                f'checking {service} on {device.host}')
    ...
    def createMetrics(self):
        self.metrics['wlanstatus'] = GaugeMetricFamily(
            'fritz_wifi_status', 'Status of WiFi',
            labels=[
                'serial', 'friendly_name', 'enabled', 'standard',
                'ssid', 'wifi_index', 'wifi_name'])
        self.metrics['wlanchannel'] = GaugeMetricFamily(
            'fritz_wifi_channel', 'Channel of WiFi',
            labels=[
                'serial', 'friendly_name', 'enabled', 'standard',
                'ssid', 'wifi_index', 'wifi_name'])
        self.metrics['wlanassocs'] = GaugeMetricFamily(
            'fritz_wifi_associations', 'Number of associations (devices) in WiFi',
            labels=[
                'serial', 'friendly_name', 'enabled', 'standard', 'ssid',
                'wifi_index', 'wifi_name'], unit='count')
        self.metrics['wlanpackets'] = CounterMetricFamily(
            'fritz_wifi_packets', 'Amount of packets in WiFi',
            labels=[
                'serial', 'friendly_name', 'enabled', 'standard',
                'ssid', 'direction', 'wifi_index', 'wifi_name'], unit='count')
        ...
        for index, wlan in enumerate(device.capabilities[self.__class__.__name__].wifi_present):
            ...
            if wlan:
                ...
                self.metrics['wlanstatus'].add_metric(
                    [
                        device.serial, device.friendly_name, wlan_enabled,
                        wlan_result['NewStandard'], wlan_result['NewSSID'],
                        str(index + 1), self.wifi_type[index]
                    ],
                    wlan_status
                )
                self.metrics['wlanchannel'].add_metric(
                    [
                        device.serial, device.friendly_name,
                        wlan_enabled, wlan_result['NewStandard'],
                        wlan_result['NewSSID'], str(index + 1), self.wifi_type[index]
                    ],
                ...
                self.metrics['wlanpackets'].add_metric(
                    [
                        device.serial, device.friendly_name,
                        wlan_enabled, wlan_result['NewStandard'],
                        wlan_result['NewSSID'], 'rx', str(index + 1), self.wifi_type[index]
                    ],
                ...
                self.metrics['wlanpackets'].add_metric(
                    [
                        device.serial, device.friendly_name,
                        wlan_enabled, wlan_result['NewStandard'],
                        wlan_result['NewSSID'], 'tx', str(index + 1), self.wifi_type[index]
                    ],
